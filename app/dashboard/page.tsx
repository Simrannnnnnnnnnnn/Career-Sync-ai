...
function StatCard({ icon, label, value, color, bg, loading }: {
  icon: string; label: string; value: string; color: string; bg: string; loading?: boolean
}) {
  return (
    <div style={{
      borderRadius: 'var(--radius-card)', padding: 'clamp(14px, 3vw, 18px) clamp(14px, 3vw, 20px)',
    ...
    >
      <div style={{
        width: 44, height: 44, borderRadius: 14, flexShrink: 0, background: bg,
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        fontSize: 20, border: `1px solid ${color}22`,
      }}>
        {icon}
      </div>
      <div style={{ minWidth: 0 }}>
        <p style={{
          fontSize: 22, fontWeight: 800, color: 'var(--text-primary)', margin: 0, lineHeight: 1,
          opacity: loading ? 0.3 : 1, transition: 'opacity 0.2s',
        }}>
          {loading ? '—' : value}
        </p>
        <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: '4px 0 0', whiteSpace: 'nowrap' }}>{label}</p>
      </div>
    </div>
  )
}
...
function FeatureTile({ f }: { f: typeof FEATURES[0] }) {
  return (
    <Link href={f.href} style={{ textDecoration: 'none' }}>
      ...
      >
        <div style={{
          width: 48, height: 48, borderRadius: 14, flexShrink: 0, background: f.bg,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: 22, border: `1px solid ${f.color}22`,
        }}>
          {f.icon}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 3, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)' }}>{f.title}</span>
            {f.badge && (
              <span style={{
                fontSize: 9, fontWeight: 800, padding: '2px 7px', borderRadius: 99,
                letterSpacing: '0.06em', color: f.badgeColor, background: f.badgeBg,
          ...
          <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: 0, lineHeight: 1.45 }}>{f.desc}</p>
        </div>
...
export default function Dashboard() {
  ...
  async function fetchStats(userId: string) {
    ...
    try {
      ...
      // Jobs applied — optional table, fail silently if missing
      try {
        const { count } = await supabase
          .from('jobs')
          .select('*', { count: 'exact', head: true })
          .eq('user_id', userId)
        if (typeof count === 'number') setJobsApplied(count)
      } catch { /* jobs table may not exist yet */ }

      // Resume score — optional, from profile or resumes table
      try {
        const { data: resumeData } = await supabase
          ...
          .limit(1)
          .single()
        if (resumeData?.ats_score) setResumeScore(resumeData.ats_score)
      } catch { /* resumes table may not exist yet */ }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding: 'clamp(16px, 5vw, 48px) clamp(12px, 4vw, 32px) 120px',
        ...
        <div style={{
          ...
          marginBottom: 'clamp(20px, 4vw, 36px)', gap: 12,
        }}>
          <div style={{ minWidth: 0 }}>
            <p style={{
            ...
            <h1 style={{
              fontSize: 'clamp(20px, 5vw, 30px)', fontWeight: 800, letterSpacing: '-0.03em',
        ...
        <div style={{
          ...
          <div style={{
            borderRadius: 'var(--radius-card)', padding: 'clamp(18px, 4vw, 28px)',
            background: 'var(--bg-card)', border: '1px solid var(--border)',
            ...
            <p style={{
              ...
              color: 'var(--indigo)', marginBottom: 10, marginTop: 8,
            }}>
              Your workspace
            </p>
            <h2 style={{
              fontSize: 'clamp(22px, 5vw, 34px)', fontWeight: 900, letterSpacing: '-0.04em',
              margin: '0 0 8px', lineHeight: 1, color: 'var(--text-primary)',
            }}>
              CareerSync AI
            </h2>
            ...
            <div style={{
              ...
              {['Resume', 'AI Interviews', 'Mock Bank', 'Job Tracker', 'Growth'].map(tag => (
                <span key={tag} style={{ fontSize: 11, color: 'var(--text-faint)' }}>✦ {tag}</span>
              ))}
            </div>
          </div>
          ...
          <div style={{
            borderRadius: 'var(--radius-card)', padding: 'clamp(18px, 4vw, 28px)',
            background: 'var(--bg-card)', border: `1px solid ${vibe.border}`,
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
              ...
              <div>
                <p style={{
                  ...
                  color: vibe.color, marginBottom: 7,
                }}>
                  {vibe.label}
                </p>
                <p style={{ color: 'var(--text-primary)', fontSize: 15, lineHeight: 1.65, fontWeight: 500, margin: 0 }}>
                  {vibe.message}
                </p>
              </div>
            </div>
            {tip && (
              <div style={{
                ...
                <p style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.55, margin: 0 }}>{tip}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{
          display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(280px, 100%), 1fr))', gap: 10,
        }}>
        ...
        {profile && (
          <div style={{
            marginTop: 20, borderRadius: 'var(--radius-card)', padding: '14px 18px',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{
                width: 40, height: 40, borderRadius: 12, background: 'var(--blue-bg)',
                border: '1px solid var(--blue)30', display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontWeight: 800, fontSize: 14, color: 'var(--blue)', flexShrink: 0,
              }}>
                {initials}
              </div>
              <div style={{ minWidth: 0 }}>
            ...
            <Link
              ...
              style={{
                ...
                background: 'var(--bg-subtle)', transition: 'all 0.2s', whiteSpace: 'nowrap',
              }}
            >
              Edit profile
            </Link>
