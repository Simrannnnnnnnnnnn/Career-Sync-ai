...
function ReportInner() {
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: '#050507', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24, fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ display: 'flex', gap: 8 }}>
        {[0,1,2].map(i => <div key={i} style={{ width: 10, height: 10, borderRadius: '50%', background: '#10b981', animation: `bounce 0.9s ease-in-out ${i * 0.18}s infinite alternate` }} />)}
      </div>
  ...
  if (error) return (
    <div style={{ minHeight: '100vh', background: '#050507', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, fontFamily: "'DM Sans', sans-serif", padding: 24 }}>
      ...
      <button onClick={() => router.push('/dashboard')} style={{ marginTop: 8, padding: '12px 28px', borderRadius: 12, border: 'none', background: 'linear-gradient(135deg, #10b981, #059669)', color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
        Back to Dashboard
      </button>
    </div>
  )
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#050507', fontFamily: "'DM Sans', 'Segoe UI', sans-serif", color: '#f4f4f5', padding: '32px 24px 64px' }}>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 16px', borderRadius: 99, border: '1px solid rgba(16,185,129,0.3)', background: 'rgba(16,185,129,0.06)', marginBottom: 16 }}>
            <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#10b981' }} />
            <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.15em', color: '#10b981', textTransform: 'uppercase' }}>Interview Report</span>
          </div>
          <h1 style={{ color: '#f4f4f5', fontSize: 28, fontWeight: 700, margin: '0 0 8px', letterSpacing: '-0.02em' }}>Performance Analysis</h1>
          <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: 13, margin: 0 }}>
            {role} · {roundLabel} Round · {diffLabel}
          </p>
          {saved && (
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginTop: 10, padding: '4px 12px', borderRadius: 99, background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)' }}>
              <span style={{ fontSize: 11, color: '#10b981', fontWeight: 600 }}>✓ Saved to your profile</span>
            </div>
          )}
        </div>

        {/* Verdict + Overall Score */}
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 20, padding: '32px', marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 24 }}>
          <div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '8px 18px', borderRadius: 99, background: verdictBg, border: `1px solid ${verdictColor}40`, marginBottom: 12 }}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 13, margin: 0, maxWidth: 340, lineHeight: 1.6 }}>{report.overallFeedback}</p>
          </div>
        ...
        {/* Sub Scores */}
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 20, padding: '28px 32px', marginBottom: 20, display: 'flex', justifyContent: 'space-around', flexWrap: 'wrap', gap: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
          <div style={{ background: 'rgba(16,185,129,0.04)', border: '1px solid rgba(16,185,129,0.15)', borderRadius: 20, padding: '24px' }}>
          ...
          <div style={{ background: 'rgba(245,158,11,0.04)', border: '1px solid rgba(245,158,11,0.15)', borderRadius: 20, padding: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
              <span style={{ fontSize: 18 }}>🎯</span>
        ...
        {/* Question Feedback */}
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 20, padding: '28px', marginBottom: 28 }}>
          ...
          {report.questionFeedback.map((qf, i) => (
            <div key={i} style={{ borderRadius: 14, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.06)', marginBottom: i < report.questionFeedback.length - 1 ? 12 : 0 }}>
              <button onClick={() => setExpandedQ(expandedQ === i ? null : i)}
                style={{ width: '100%', padding: '16px 18px', background: 'rgba(255,255,255,0.02)', border: 'none', cursor: 'pointer', textAlign: 'left', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, fontFamily: 'inherit' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                  <span style={{ fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 99, background: 'rgba(99,102,241,0.12)', color: '#6366f1', flexShrink: 0, marginTop: 1 }}>Q{i + 1}</span>
                  <span style={{ color: '#e4e4e7', fontSize: 13, fontWeight: 600, lineHeight: 1.5 }}>{qf.question}</span>
                </div>
                <span style={{ color: 'rgba(255,255,255,0.3)', fontSize: 16, flexShrink: 0 }}>{expandedQ === i ? '▲' : '▼'}</span>
              </button>
              {expandedQ === i && (
                <div style={{ padding: '0 18px 18px', background: 'rgba(0,0,0,0.2)' }}>
                  <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 10, padding: '12px 14px', marginBottom: 10, borderLeft: '2px solid rgba(59,130,246,0.5)' }}>
                  ...
                  <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 10, padding: '12px 14px', borderLeft: '2px solid rgba(16,185,129,0.5)' }}>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          <button onClick={() => router.push('/interview/setup')}
            style={{ flex: 1, minWidth: 160, padding: '14px', borderRadius: 14, border: 'none', background: 'linear-gradient(135deg, #10b981, #059669)', color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
            🔄 Retake Interview
          </button>
          <button onClick={() => router.push('/dashboard')}
            style={{ flex: 1, minWidth: 160, padding: '14px', borderRadius: 14, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.03)', color: 'rgba(255,255,255,0.6)', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
            🏠 Dashboard
          </button>
        </div>
      </div>
...
export default function ReportPage() {
  return (
    <Suspense fallback={<div style={{ background: '#050507', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ color: 'rgba(255,255,255,0.3)', fontFamily: 'sans-serif' }}>Loading…</div></div>}>
      <ReportInner />
    </Suspense>
  )
}